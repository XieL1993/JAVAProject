!function (a) {
    ...
    var b = {
        ...
        i = ["laydate_only_year", "laydate_year_month", "laydate_only_month"], j = ["今天", "本年", "本月"];
    a.laydate = function (b) {
        ...
        var e, f, i, d = c.query, g = h.event;
        try {
            i = g.target || g.srcElement || {}
        } catch (j) {
            i = {}
        }
        if (e = a.elem ? d(a.elem) : i, h.elemv = /textarea|input/.test(e.tagName.toLocaleLowerCase()) ? "value" : "innerHTML", g && i.tagName) {
            if (!e || e === c.elem) return;
            c.stopMosup(g.type, e), c.stopmp(g), c.view(e, a), c.reshow()
        } else f = a.event || "click", c.each((0 | e.length) > 0 ? e : [e], function (b, d) {
            c.stopMosup(f, d), c.on(d, f, function (b) {
        ...
        if ("" !== d.replace(/\s/g, "")) {
            if ("MM" == c.options.format) return 1;
            if (!b.test(d)) return c.elem[h.elemv] = "", c.msg("日期不符合格式，请重新选择。"), 1;
            if (f[0]) return c.elem[h.elemv] = "", c.msg("日期不在有效期内，请重新选择。"), 1;
            f.value = c.elem[h.elemv].match(b).join(), e = f.value.match(/\d+/g), e[1] < 1 ? (e[1] = 1, f.auto = 1) : e[1] > 12 ? (e[1] = 12, f.auto = 1) : e[1] && e[1].length < 2 && (f.auto = 1), e[2] < 1 ? (e[2] = 1, f.auto = 1) : e[2] > c.months[(0 | e[1]) - 1] ? (e[2] = 31, f.auto = 1) : e[2] && e[2].length < 2 && (f.auto = 1), e.length > 3 && (c.timeVoid(e[3], 0) && (f.auto = 1), c.timeVoid(e[4], 1) && (f.auto = 1), c.timeVoid(e[5], 2) && (f.auto = 1)), f.auto ? c.creation([e[0], 0 | e[1], 0 | e[2]], 1) : f.value !== c.elem[h.elemv] && (c.elem[h.elemv] = f.value)
        }
        ...
        }), b("#laydate_ys").innerHTML = d, c.each(b("#laydate_ys li"), function (a, d) {
            "y" === c.checkVoid(d.getAttribute("y"))[0] ? c.addClass(d, h[1]) : c.on(d, "click", function (a) {
                c.each(b("#laydate_ys li"), function (a, b) {
                    c.removeClass(b, h[2])
                }), c.addClass(d, h[2]), c.stopmp(a).reshow(), c.viewDate(0 | this.getAttribute("y"), c.ymd[1], c.ymd[2]), c.options.onlyY && b("#laydate_ok").click()
            })
        })
    }, c.initDate = function () {
        var d = (c.query, new Date), e = c.elem[h.elemv].match(/\d+/g) || [];
        if (e.length < 3) switch (e.length) {
            case 2:
                e[2] = 1;
                break;
            case 1:
                /\d{4}/.test(e[0]) ? (e[1] = 1, e[2] = 1) : (e.unshift(d.getFullYear()), e.push(1));
                break;
            case 0:
        ...
        a.style.left = e.left + (b ? 0 : c.scroll(1)) + "px", d = e.bottom + a.offsetHeight / 1.5 <= c.winarea() ? e.bottom - 1 : e.top > a.offsetHeight / 1.5 ? e.top - a.offsetHeight + 1 : c.winarea() - a.offsetHeight, a.style.top = Math.max(d + (b ? 0 : c.scroll()), 1) + "px"
    }, c.spcls = function () {
        return h[0] + (c.options.cls ? " " + c.options.cls : "") + (c.options.onlyY ? " " + i[0] : "") + (c.options.onlyYM ? " " + i[1] : "") + (c.options.onlyM ? " " + i[2] : "")
    }, c.follow = function (a) {
        var b, d;
        a.className = c.spcls(), b = c.changeTodayText(), d = c.query, d("#laydate_today").innerHTML = b, c.options.fixed ? (a.style.position = "fixed", c.orien(a, 1)) : (a.style.position = "absolute", c.orien(a))
    }, c.changeTodayText = function () {
        var a = j[0];
        return c.options.onlyY && (a = j[1]), c.options.onlyM && (a = j[2]), c.options.onlyYM && (a = j[0]), a
    }, c.viewtb = function () {
        ...
        var i, g = c.query, k = {}, l = j[0];
        f = f || a, c.elem = a, c.options = f, c.options.format || (c.options.format = b.format), c.options.start = c.options.start || "", c.mm = k.mm = [c.options.min || b.min, c.options.max || b.max], c.mins = k.mm[0].match(/\d+/g), c.maxs = k.mm[1].match(/\d+/g), l = c.changeTodayText(), c.box ? c.shde(c.box) : (i = d[e]("div"), i.id = h[0], i.className = c.spcls(), i.style.cssText = "position: absolute;", i.setAttribute("name", "laydate-v" + laydate.v), i.innerHTML = k.html = '<div class="laydate_top"><div class="laydate_ym laydate_y" id="laydate_YY"><a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a><input id="laydate_y" readonly><label></label><a class="laydate_choose laydate_chnext laydate_tab"><cite></cite></a><div class="laydate_yms"><a class="laydate_tab laydate_chtop"><cite></cite></a><ul id="laydate_ys"></ul><a class="laydate_tab laydate_chdown"><cite></cite></a></div></div><div class="laydate_ym laydate_m" id="laydate_MM"><a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a><input id="laydate_m" readonly><label></label><a class="laydate_choose laydate_chnext laydate_tab"><cite></cite></a><div class="laydate_yms" id="laydate_ms">' + function () {
            var a = "";
            return c.each(new Array(12), function (b) {
                a += '<span m="' + b + '">' + c.digit(b + 1) + "月</span>"
            }), a
        }() + "</div>" + "</div>" + "</div>" + c.viewtb + '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">时间</li>' + "<li><input readonly>:</li>" + "<li><input readonly>:</li>" + "<li><input readonly></li>" + "</ul>" + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>' + '<a id="laydate_today">' + l + "</a>" + '<a id="laydate_ok">确认</a>' + "</div>" + (b.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + "</a>" : "") + "</div>", d.body.appendChild(i), c.box = g("#" + h[0]), c.events(), i = null), c.follow(c.box), f.zIndex ? c.box.style.zIndex = f.zIndex : c.removeCssAttr(c.box, "z-index"), c.stopMosup("click", c.box), c.initDate(), c.iswrite(), c.check(), (c.options.onlyY || c.options.onlyYM) && g("#laydate_y").click()
    }, c.reshow = function () {
        ...
        return a = a.concat(d), e = e || (c.options ? c.options.format : b.format), "MM" === e ? c.digit(a[1]) : e.replace(/YYYY|MM|DD|hh|mm|ss/g, function () {
        ...
        c.addClass(d.body, "laydate_body"), h.tds = b("#laydate_table td"), h.yys = b("#laydate_ys li"), h.mms = b("#laydate_ms span"), h.year = b("#laydate_y"), h.month = b("#laydate_m"), c.each(b(e.box + " .laydate_ym"), function (a, b) {
        ...
        }), c.each(b("#laydate_ms span"), function (a, d) {
            c.on(d, "click", function (a) {
                c.stopmp(a).reshow(), c.hasClass(this, h[1]) || (c.viewDate(c.ymd[0], 0 | this.getAttribute("m"), c.ymd[2]), c.options.onlyM && b("#laydate_ok").click())
            })
            ...
            b = b || a.event;
            var d = b.keyCode;
            13 === d && c.elem && c.creation([c.ymd[0], c.ymd[1] + 1, c.ymd[2]])
        })
    }, c.init = function () {
        c.use("need"), c.use(h[4] + b.skin, h[3]), c.skinLink = c.query("#" + h[3])
